...
namespace KagContext.parse
{
	...
	public class ParserService
	{
		...
		/// </summary>
		static readonly List<string> SUPPORTED_EXT = new List<string> { "*.tjs", "*.ks" };
        ...
        /// スリープタイム
        /// </summary>
        private int m_codeComplateSeepTime = 1500;
		#endregion
		...
		void BackThreadParser_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			if (arg.ModeType == ParserArg.Mode.Directory)	//ディレクトリモードの時
			{
				...
				{
					count++;
                    using (sr = new StreamReader(path, TextEncoding.Instance.GetFileEncoding(path)))
					{
						text = sr.ReadToEnd();
					}
		...
		public void ParseDirectory(string dirPath, string[] exAddPathList)
		{
			//パラメーターチェック
            ...
            list.AddRange(exAddPathList);
            /*
            switch (projectName)
			{
				case "kag3":
					list.Add(EnvPath.Instance.KagDefFilePath);
					break;
            ...
            */
			...
			//ファイルパスを取得する
			string[] pathList = FileUtil.GetDirectoryFile(dirPath, SUPPORTED_EXT, SearchOption.AllDirectories);
			if (pathList == null)
			{
				if (list.Count == 0)
				{
			...
			if (m_bgworker.IsBusy == false)
			{
				//パース開始
				DebugTimer.Start();
                m_bgworker.RunWorkerAsync(new DirectoryParserArg(list.ToArray(), m_codeComplateSeepTime));
			}
			else
			{
		...
		public KagMacro[] GetKagMacroList()
		{
			...
			//KAG3
            if (m_cuManger.KagTable.ContainsKey(EnvPath.Instance.KagDefFilePath))
			{
				//KAG3
                foreach (KagMacro macro in m_cuManger.KagTable[EnvPath.Instance.KagDefFilePath].MacroTable.Values)
				{
