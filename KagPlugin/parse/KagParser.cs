...
namespace KagContext.parse
{
	...
	class KagParser : IParser
	{
		...
		/// <returns></returns>
		private string[] getMacroAttrNameFromMp(string val)
		{
			List<string> attrNameList = new List<string>();
			System.Text.RegularExpressions.MatchCollection mc = System.Text.RegularExpressions.Regex.Matches(val, @"mp\.([a-zA^Z0-9]+)");
			foreach (System.Text.RegularExpressions.Match m in mc)
			{
				if (m.Groups.Count >= 2)
				{
					//()に囲まれた範囲のみを取得する
					attrNameList.Add(m.Groups[1].Value);
				}
			}

			return attrNameList.ToArray();
		}
