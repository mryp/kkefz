	...
	public static class KagUtility
	{
		...
		public static KagTagKindInfo GetTagKind(string line)
		{
			...
			for (int i = 0; i < line.Length; i++)
			{
				...
				{
					startFlag = false;
                    if (stringModeFlag == false && info.Kind == KagTagKindInfo.KagKind.Unknown)
					{
                        info.Kind = KagTagKindInfo.KagKind.KagTagName;
						info.TagName = "";
						info.AttrName = "";
						info.AttrValue = "";
				...
				else if (line[i] == ']')
				{
					...
					{
                        info.Kind = KagTagKindInfo.KagKind.Unknown;
						info.TagName = "";
						info.AttrValue = "";
						info.AttrValue = "";
					}
				...
				else if (line[i] == ' ')
				{
					...
					switch (info.Kind)
					{
                        ...
                        case KagTagKindInfo.KagKind.AttrValue:
							if (stringModeFlag == false)
							{
                                info.Kind = KagTagKindInfo.KagKind.AttrName;
				...
				else if (line[i] == '=')
				{
					...
					{
                        case KagTagKindInfo.KagKind.AttrName:
                            info.Kind = KagTagKindInfo.KagKind.AttrValue;
							info.AddAttrTable(info.AttrName, info.AttrValue);
							info.AttrValue = "";
							break;
						default:
            ...
            if (info.Kind == KagTagKindInfo.KagKind.AttrName
            || info.Kind == KagTagKindInfo.KagKind.AttrValue)
			{
				info.AddAttrTable(info.AttrName, info.AttrValue);
			}
        ...
        /// <summary>
        /// 現在のカーソル位置にあるKAGのタグ一式分の行文字列を取得する
        /// </summary>
        /// <param name="sci"></param>
        /// <returns></returns>
        public static string GetKagLineText(ScintillaNet.ScintillaControl sci, int pos)
        {
            int curLine = sci.LineFromPosition(pos);
            int firstPos = sci.PositionFromLine(curLine);
            int curPos = pos;
            string lineText = "";
            if ((curPos - firstPos) > 0)
            {
                int startPosition = sci.MBSafeCharPosition(firstPos);
                int endPosition = sci.MBSafeCharPosition(curPos);
                lineText = sci.GetLine(curLine).Substring(0, endPosition - startPosition);
                //lineText = sci.GetCurLine(curPos - firstPos);
            }
